...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_topics'])) {
    ...
    // Process subtopic updates if they exist
    if(isset($_POST['subtopic_status'])) {
        foreach ($_POST['subtopic_status'] as $subtopic_id => $status) {
            // Check if a record already exists
            $check_query = "SELECT id FROM student_subtopic_progress WHERE student_id = ? AND subtopic_id = ?";
            $stmt = $conn->prepare($check_query);
            $stmt->bind_param("ii", $student_id, $subtopic_id);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            if ($result->num_rows > 0) {
                // Update existing record
                $update_query = "UPDATE student_subtopic_progress
                                SET is_completed = ?,
                                    completed_date = ?,
                                    tutor_id = ?,
                                    updated_at = CURRENT_TIMESTAMP
                ...
                $stmt = $conn->prepare($update_query);
                $stmt->bind_param("isiii", $is_completed, $completed_date, $tutor_id, $student_id, $subtopic_id);
            } else {
                // Insert new record
                $insert_query = "INSERT INTO student_subtopic_progress
                                (student_id, subtopic_id, is_completed, completed_date, tutor_id)
                                VALUES (?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($insert_query);
                $stmt->bind_param("iiisi", $student_id, $subtopic_id, $is_completed, $completed_date, $tutor_id);
            }
    ...
    // Set success message
    $_SESSION['success_message'] = "Topic and subtopic progress updated successfully!";
...
<head>
    ...
    <script>
        function toggleSubtopics(topicId) {
            const subtopics = document.querySelectorAll('.subtopic-' + topicId);
            for (let subtopic of subtopics) {
                subtopic.style.display = subtopic.style.display === 'none' ? 'table-row' : 'none';
            }
        }
    </script>
</head>
   ...
   <div class="container">
       ...
       if ($subjects_result->num_rows === 0) {
           ...
           while ($subject = $subjects_result->fetch_assoc()) {
               ...
               echo "<h2>" . htmlspecialchars($subject_name ?? '') . " - " . htmlspecialchars($year_level ?? '') . "</h2>";
               ...
               if ($topics_result->num_rows > 0) {
                   ...
                   while ($topic = $topics_result->fetch_assoc()) {
                       ...
                       // Get subtopics count for this topic
                       $subtopics_count_query = "SELECT COUNT(*) as count FROM subtopics WHERE topic_id = ?";
                       $stmt = $conn->prepare($subtopics_count_query);
                       $stmt->bind_param("i", $topic_id);
                       $stmt->execute();
                       $subtopics_count_result = $stmt->get_result();
                       $subtopics_count = $subtopics_count_result->fetch_assoc()['count'];
                       
                       echo "<tr class='topic-row collapsible' onclick='toggleSubtopics($topic_id)'>";
                       echo "<td>" . htmlspecialchars($topic['topic_name'] ?? '') . " <span class='badge'>($subtopics_count)</span></td>";
                       ...
                       $subtopics_query = "SELECT s.subtopic_id, s.subtopic_name, s.description, 
                                          s.difficulty_level, s.estimated_time_minutes,
                                          IFNULL(ssp.is_completed, 0) as is_completed,
                                          ssp.completed_date
                                      FROM subtopics s
                                      LEFT JOIN student_subtopic_progress ssp 
                                          ON s.subtopic_id = ssp.subtopic_id AND ssp.student_id = ?
                                      WHERE s.topic_id = ?
                                      ORDER BY s.subtopic_id";
                       $stmt = $conn->prepare($subtopics_query);
                       $stmt->bind_param("ii", $student_id, $topic_id);
                       $stmt->execute();
                       $subtopics_result = $stmt->get_result();
                       
                       while ($subtopic = $subtopics_result->fetch_assoc()) {
                           $subtopic_id = $subtopic['subtopic_id'];
                           $is_subtopic_checked = $subtopic['is_completed'] ? 'checked' : '';
                           $subtopic_completed_date = $subtopic['completed_date'] ? date('M d, Y', strtotime($subtopic['completed_date'])) : '-';
                           $difficulty = htmlspecialchars($subtopic['difficulty_level'] ?? '');
                           $time = (int)($subtopic['estimated_time_minutes'] ?? 0);
                           
                           echo "<tr class='subtopic-row subtopic-$topic_id' style='display: none;'>";
                           echo "<td class='subtopic-name'>" . htmlspecialchars($subtopic['subtopic_name'] ?? '') . 
                                " <span class='subtopic-info'>(Level: $difficulty, $time mins)</span></td>";
                           echo "<td>" . htmlspecialchars($subtopic['description'] ?? '') . "</td>";
                           echo "<td class='checkbox-cell'>";
                           echo "<label class='switch'>";
                           echo "<input type='checkbox' name='subtopic_status[{$subtopic_id}]' value='completed' {$is_subtopic_checked}>";
                           echo "<span class='slider round'></span>";
                           echo "</label>";
                           echo "</td>";
                           echo "<td>{$subtopic_completed_date}</td>";
                           echo "</tr>";
                       }
                   }
